...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/18 08:27:06 by cmira-am          #+#    #+#             */
/*   Updated: 2023/06/01 13:08:32 by cmira-am         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_get_rest(char *str)
{
	...
	rest_str = malloc((str_lenght - count) * sizeof(char));
	if (rest_str == NULL)
		return (NULL);
	count++;
	rest_count = 0;
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	next_string = ft_get_lines(fd, next_line);
	if (next_string == NULL)
	{
		if (next_line)
		{
	...
	next_line = next_string;
	actual_line = NULL;
	actual_line = ft_actual_line(next_line, actual_line);
	next_line = ft_get_rest(next_line);
	return (actual_line);
}
